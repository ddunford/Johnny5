...
- **The Conscience vets ACTIONS, not cognition** (Phase 6). Only Effector *tool* actions (world/state-touching) go through the vet→run→audit dispatch (FC-5) + the append-only `action_log`. Internal cognition (reflect/recall/narrate) rides the bus, NOT vetted — vetting thought would be the wrong shape, and a fail-closed Conscience would stop Johnny *thinking* when local inference is down. The line: **if it acts on the world or persistent state, it's a `Tool`** (auto-vetted by being on the belt); if it only produces a thought, it isn't.
- **The curiosity loop eases on the *consolidation*, not the fetch** (Phase 6b). A web read isn't "done" — and doesn't ease Curiosity — until `WebReadConsolidator` summarises it into an episode + semantic fact (provenance = url). Reading without remembering eases nothing; that's what makes browsing *grow* him rather than dump pages into the workspace.
- **web_fetch SSRF guard pins the connection to the validated IP** (Phase 6b, `brain/effectors/safe_http.py`). Johnny picks URLs autonomously, so the guard resolves DNS, blocks if *any* resolved IP is non-global, then connects to the *validated IP* (not the hostname — defeats DNS-rebinding) with the real Host/SNI preserved, re-validating every redirect hop. `code_exec` reaches its hardened sandbox via a **scoped `docker-socket-proxy`** (the api never holds the raw socket). SearXNG: pass explicit `engines=google,bing,brave` (duckduckgo returns 0 from `inference.lan`) / `categories=news` (see `plan/inference-substrate.md`).
