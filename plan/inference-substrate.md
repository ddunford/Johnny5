...
## Open follow-up — qwen `/no_think` for structured roles (Phase 6)

The qwen3.5 reasoning preamble is expensive on **both** axes for structured (schema) output: it eats `max_tokens` (the truncation that bit the Phase-4 sleep roles — fixed by raising those to 4096) **and** wall-clock (a self-model refresh on the qwen fallback took ~150 s — handled in Phase 4 by a per-model `LOCAL_REASONING_TIMEOUT=240` applied only to the reasoning model, so the per-tick fast model keeps its tight budget). SPEC §10 says **"always `/no_think` for structured output"** on local Qwen — if honoured, qwen would skip the preamble entirely, making structured calls fast + compact and removing the need for the big token ceiling + wide timeout on the fallback path.

**Status (RESOLVED 2026-05-27, Phase 6a):** the prompt-token `/no_think` form does **not** work for `qwen3.5-9b-128k:latest`. Crucially, the `think` field on the **OpenAI-compat `/v1/chat/completions`** endpoint is also **ignored** (probed live: reasoning preamble still emitted, `content` empty, `finish=length`). The mechanism that **works** is Ollama's **native `/api/chat`** endpoint with top-level `think: false` + `format: "json"` + `stream: false` → clean JSON `content`, `done_reason=stop`, zero `thinking` (probed live: returned `{"verdict":"allow","reason":"ok"}` directly). So `OllamaProvider.complete()` routes the **reasoning model on a schema (json_object) role** to the native endpoint with thinking off; everything else (the fast model, free-text reasoning) keeps the `/v1` path. Native response shape differs: thinking channel is `message.thinking`, token counts are `prompt_eval_count`/`eval_count`, stop reason is `done_reason` — parsed by `parse_native_chat()`. Proven by `tests/llm/test_no_think_live.py` (`@live`). The 4096 ceiling + 240 s reasoning timeout are now belt-and-suspenders for the (rare) free-text reasoning path, not load-bearing for structured roles.
