...
**Status (probed 2026-05-27):** appending `/no_think` to the prompt did **not** suppress the reasoning chain for the `qwen3.5-9b-128k:latest` tag (it still emitted chain-of-thought). The real mechanism needs nailing down — likely an Ollama `think: false` request option / a model-specific param rather than a prompt token. **Phase 6 (inference-substrate / router):** determine the correct `/no_think` mechanism for this tag, wire it into the Ollama adapter for schema roles, then the 4096 ceiling + 240 s reasoning timeout become belt-and-suspenders rather than load-bearing. Until then, the token bump + per-model timeout are the correct, verified fix.
