...
- [x] `TASK-5a.10` ⫘ Security review: the token gate actually protects every `/api/v1` route (no unauthenticated leak); `/input` can't inject control commands or oversized payloads; no secrets in any envelope or the audit log; health redaction holds → `/security-reviewer` [TC-5a.1]
  - Verdict: **PASS — no findings.** The gate is applied at the `v1_router` level so all 8 sub-routers inherit it (no per-route opt-out); constant-time compare, header-based (not URL), token never logged, blank=dev-open. `/input` is bounded (blank→422 validator, oversized→413, full→429), pushes only text to the shared queue (can't inject control commands — those are a separate token-gated channel) and doesn't bypass the cycle. Every GET is read-only (zero mutation paths, grep-confirmed). No secrets in any envelope or the `workspace_event`/audit payloads. `/api/health` redaction holds. Two **forward** advisories (not 5a issues): (1) Phase 6+ must keep secrets out of `workspace_event` payloads — `/audit` now exposes them to token-holders; (2) 5b's browser WebSocket will carry `?token=` in the URL (browsers can't set WS headers) — an inherent interim-gate limitation, mitigated by `wss://`, real fix is the later session/cookie gate.
