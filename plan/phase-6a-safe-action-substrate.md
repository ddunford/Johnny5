...
- **FC-9 — the Conscience lives in the Mind, NOT the Core.** The Conscience judges the *content/values* of an action (Johnny-editable, prompt-backed, FC-3). The Core only enforces continuity + host safety (budget/sandbox/audit). Do NOT put values-vetting in `core/`. The Core's contribution here is the *append-only audit writer* + the *budget gate* (mechanism, not judgement).
...
- `Conscience.vet(action, *, values, workspace) -> Verdict` — judges a proposed `(tool, args)` against Johnny's values (git-backed prompt, FC-3) + **hard invariants** (a small allowlist/denylist the Conscience can tighten but a Phase-6 default forbids the obviously-harmful). Returns allow / veto + reason. Router role `conscience` (local/fast — `SPEC §10`). Pure projection `parse_verdict` for the contract test.
...
- [ ] `TASK-6a.3` Conscience agent: `vet((tool,args)) -> Verdict(allow|veto, reason)` against values (prompt, FC-3) + hard-invariant defaults; `conscience` role (local) + `config/prompts/conscience.md`; pure `parse_verdict`; fills the cycle CHECK stub (FC-7/FC-9) → `/fastapi-engineer` [TC-6a.2, TC-6a.8]
- [ ] `TASK-6a.4` `EffectorDispatch` on the FC-5 `_dispatch_action` point: vet → (allow) run via registry → write `action_log` → emit; (veto) log + emit, never run. Wire CHECK+ACT; Deliberation may propose a `(tool,args)` action (inert tool this phase) → `/fastapi-engineer` [TC-6a.3, TC-6a.4]
...
- [ ] `TASK-6a.9` ⫘ Tests: Conscience allow/veto (+ hard-invariant veto), dispatch vet→run→audit + vet→veto→no-run, action_log shape + append-only, budget hard-gate skips cloud when exhausted, no-secret-leak redaction → `/qa-test-engineer` [TC-6a.1..6a.5, 6a.7]
...
- [ ] `TASK-6a.11` ⫘ Security review: every goal→tool path passes through the vet+audit point (no bypass); the Conscience hard-invariant default actually vetoes harmful actions; `core/audit.py` is append-only + import-isolated (Mind can't erase the trail, FC-1); the budget gate genuinely blocks cloud spend when exhausted; the no-secrets guard holds on both write paths → `/security-reviewer` [TC-6a.5, TC-6a.7]
