...
- [ ] `TASK-6b.5` `WebReadConsolidator` — fetched/searched content → summarised into episodic + semantic memory (reuse the Phase-4 `consolidation` summariser, provenance=url); bounded raw text into the workspace → `/fastapi-engineer` [TC-6b.4]
- [ ] `TASK-6b.6` `code_exec` sandbox: a hardened container image (no host mount, `--network none`/egress-allowlist, cpu/mem/pids caps, non-root, hard timeout) + the tool that dispatches into it; `ctl.sh` builds it → `/devops-deployment-engineer` + `/fastapi-engineer` [TC-6b.7, TC-6b.10]
...
- [ ] `TASK-6b.12` ⫘ Contract + `@live`: tool-result projections; a `@live` SearXNG round-trip (real `inference.lan:8889`) + a `@live` sandbox exec (real container) → `/qa-test-engineer` [TC-6b.1, TC-6b.7]
...
- [ ] `TASK-6b.14` Surface the **durable `action_log` trail** end-to-end (backend read + frontend), now that 6b's real tools generate real rows. 6a established the durable trail's integrity (Core-written `action_log`, append-only, no Mind delete, redacted) and surfaced actions on `/api/v1/audit` via the live `workspace_event` bus feed; it deliberately did NOT add a dedicated durable-trail API read (no rows + no consumer in a backend-only phase). 6b adds: (a) a token-gated API read of the durable trail from `ActionLogRepository` (`actions` field or `GET /api/v1/audit/actions`); (b) the AuditPanel rendering it, pinned with a captured-wire contract test (house rule). → `/fastapi-engineer` + `/frontend-react-architect` [TC-6b.11]
