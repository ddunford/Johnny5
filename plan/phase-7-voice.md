...
- [ ] `TASK-7.5` Integrate the existing `voice/` robot-voice DSP into the speak loop (TTS bytes → DSP → audio out via the device). → `/fastapi-engineer` + `/devops-deployment-engineer` [TC-7.5]
